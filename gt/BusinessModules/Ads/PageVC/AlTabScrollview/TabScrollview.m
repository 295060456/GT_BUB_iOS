...
-(void)configParameter:(DirectionStyle)directionStyle viewArr:(NSArray<UIView*>*)viewArr tabWidth:(NSInteger)tabWidth tabHeight:(NSInteger)tabHeight index:(NSInteger)index block:(TabClickBlock) clickBlock{
    ...
    if(_direction==horizontal){
        ...
        [_viewArr enumerateObjectsUsingBlock:^(UIView * _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
            obj.frame=CGRectMake(idx*weakSelf.tabWidth, weakSelf.tabHeight - 35, weakSelf.tabWidth, 35);
            UILabel* lab = (UILabel*)obj;
            lab.font = kTagLabTextFontNormal;
            lab.textColor = kTagLabTextColorNormal;
            [self addSubview:obj];
        ...
        [_viewArr enumerateObjectsUsingBlock:^(UIView * _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
            obj.frame=CGRectMake(0, idx*weakSelf.tabHeight, weakSelf.tabWidth, weakSelf.tabHeight);
            UILabel* lab = (UILabel*)obj;
            lab.font = kTagLabTextFontNormal;
            lab.textColor = kTagLabTextColorNormal;
            [self addSubview:obj];
    ...
    UILabel* lab = (UILabel*)self.viewArr[index];
    lab.font = kTagLabTextFontSelected;
    lab.textColor = kTagLabTextColorSelected;
}
...
-(void)autoTabOffset:(NSInteger)index{
    ...
    if(_direction==horizontal){
        //获取scrollview宽度
        NSInteger maxWidth=MAINSCREEN_WIDTH;//self.frame.size.width;
...
-(void)updateTag:(NSInteger)index{
    ...
    _tagLine.frame=CGRectMake(index*_tabWidth+kTagStringWidth/2, _tabHeight-kTagLineheight-1, _tabWidth-kTagStringWidth, kTagLineheight);
}
/**
 更新标记线位置
...
-(void)updateTagLine:(NSInteger)index{
    ...
    if(_direction==horizontal){
        ...
        [UIView animateWithDuration:0.1 animations:^{
            weakSelf.tagLine.frame=CGRectMake(index*weakSelf.tabWidth+kTagStringWidth/2, weakSelf.tabHeight-kTagLineheight-0.5, weakSelf.tabWidth-kTagStringWidth, kTagLineheight);
        ...
        //标记线切换动画
        [UIView animateWithDuration:0.1 animations:^{
            weakSelf.tagLine.frame=CGRectMake(weakSelf.tabWidth-kTagLineheight-0.5+kTagStringWidth/2, weakSelf.tabHeight*index-kTagStringWidth, kTagLineheight, weakSelf.tabHeight);
        } completion:^(BOOL finished) {
            weakSelf.tagIndex=index;
    ...
    for (int i=0; i<_viewArr.count; i++) {
        UILabel* lab = (UILabel*)self.viewArr[i];
        lab.font = kTagLabTextFontNormal;
        lab.textColor = kTagLabTextColorNormal;
    }
    UILabel* lab = (UILabel*)self.viewArr[index];
...
-(void)initView{
    ...
    self.tagLine.backgroundColor=kTagLineColor;  //默认标记线为红色
